...
export const initScrollEffects = () => {
    ...
    // Skip the hero — its copy is driven by the Framer Motion scroll sequence
    ...
    const updateScrollEffects = () => {
        ...
        // Skip sections marked data-hero — those drive their own scroll zoom
        // (and their transforms) from inside the component, so fighting them
        // here would double-scale the scene.
        const heroContainer = document.querySelector('#hero-container') || document.querySelector('section:first-of-type');
        if (heroContainer && !heroContainer.hasAttribute('data-hero')) {
